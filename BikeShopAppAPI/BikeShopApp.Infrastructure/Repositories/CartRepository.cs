...
namespace BikeShopApp.Infrastructure.Repositories
{
    public class CartRepository : ICartRepository
    {
        ...
        public async Task<bool> AddToCartAsync(int cartId, int productId)
        {
            ...
            var productAlreadyInCart = await _context.CartsProducts.AnyAsync(cp => cp.CartId == cartId && cp.ProductId == productId);
        ...
        {
            var cartsProductsEntity = await _context.CartsProducts
                .FirstOrDefaultAsync(cp => cp.ProductId == productId && cp.CartId == cartId);

            if (cartsProductsEntity == null)
            {
                return false;
            }
        ...
        public Task<bool> ProductInCartAsync(int cartId, int productId)
        {
            return _context.CartsProducts
