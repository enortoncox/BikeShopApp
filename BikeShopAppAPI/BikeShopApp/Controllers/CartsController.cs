...
namespace BikeShopApp.WebAPI.Controllers
{
    ...
    public class CartsController : CustomControllerBase
    {
        ...
        public async Task<IActionResult> AddToCart(int cartId, CartItemDto cartItemDto)
        {
            if (cartItemDto == null)
            {
                return Problem(detail: "No cart item was passed.", statusCode: 400, title: "Bad Request");
            }

            if (cartItemDto.CartId == null || cartItemDto.ProductId == null)
            {
                return Problem(detail: "Cart Id and Product Id are required.", statusCode: 400, title: "Bad Request");
            }
            ...
            if (await _cartRepository.ProductInCartAsync(cartId, cartItemDto.ProductId.Value))
            {
                return Problem(detail: "This product is already in your cart.",statusCode: 409, title: "Product Already In Cart");
            }
